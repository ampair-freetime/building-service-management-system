...
def test_create_sends_password_that_can_log_in(test_context, monkeypatch) -> None:
    ...
    assert "HK001" in sent[0]["text_body"]
    ...
    login = client.post(
        "/api/v1/auth/login",
        json={"identifier": "hk001", "password": password},
    )
    assert login.status_code == 200
...
def test_delivery_failure_keeps_account_and_reports_false(test_context, monkeypatch) -> None:
    ...
    assert [item["staff_code"] for item in listing.json()] == ["ADMIN001", "HK001"]
...
def test_duplicate_or_invalid_request_never_sends_email(test_context, monkeypatch) -> None:
    ...
    duplicate = client.post(
        "/api/v1/staff",
        headers=headers,
        json=staff_payload(staff_code="HK002", email="hk@example.com"),
    )
    assert duplicate.status_code == 409
    assert len(sent) == 1

    for extra in ({"password": "not-allowed"}, {"status": "suspended"}):
